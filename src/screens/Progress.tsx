...
import { Flame, History, Scale, TrendingUp } from 'lucide-react';
...
export function Progress() {
  ...
  useEffect(() => {
    ...
    getWorkoutHistory(4).then(entries => {
      setWorkoutHistory(entries.map(entry => ({
        date: entry.date,
        sessionType: (entry.payload as WorkoutPayload).sessionType,
      })));
    });
  }, [getWeightHistory, getWorkoutHistory]);
  ...
  const suppStreak = suppEntries.length > 0 ? 1 : 0;
      ...
      <div className="bg-slate-800 rounded-2xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-slate-700/50 rounded-xl p-3 text-center">
            <p className="text-2xl font-bold text-yellow-400">{suppStreak > 0 ? 'Yes' : '-'}</p>
            <p className="text-xs text-slate-500 mt-1">Supplements logged</p>
          </div>
          <div className="bg-slate-700/50 rounded-xl p-3 text-center">
            <p className="text-2xl font-bold text-blue-400">{todayWeight ? 'Yes' : '-'}</p>
